...
def preprocess(tokenizer, template, column_names, padding, max_src_len, max_tgt_len, examples):
...
def main():
    ...
    task_name = get_task_name(args.dataset_name, args.dataset_config_name, args.template_name)
    ...
    if args.dataset_name is not None:
        ...
        if args.dataset_name == "anli":
            ...
            raw_datasets = load_dataset('story_cloze', '2016', split='validation', data_dir='../dataset', )
        else:
    ...
    with accelerator.main_process_first():
        preprocess_fn = functools.partial(
            preprocess,
            tokenizer, template, column_names, padding, args.max_length, args.target_max_length
        )
    ...
    if accelerator.is_main_process:
        if args.output_dir is not None:
            with open(os.path.join(args.output_dir, task_name + ".json"), "w") as f:
