...
@dataclass
class T5MetroConfig(FairseqDataclass):
    data: str = field(
        default=MISSING,
        metadata={
...
@register_task("t5_metro", dataclass=T5MetroConfig)
class T5MetroTask(FairseqTask):
    cfg: T5MetroConfig

    def __init__(self, cfg: T5MetroConfig, dictionary):
        super().__init__(cfg)
    ...
    @classmethod
    def setup_task(cls, cfg: T5MetroConfig, **kwargs):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        src_dataset, tgt_dataset = MaskTokensDataset.apply_mask(
            ...
            mask_idx=self.sentinel_idx_begin,
            seed=self.cfg.seed,
        ...
        generator_target_dataset = RightPadDataset(
            tgt_dataset,
            pad_idx=self.source_dictionary.pad(),
        )
        discriminator_target_dataset = RightPadDataset(
            dataset,
            pad_idx=self.source_dictionary.pad(),
        )
            ...
            NestedDictionaryDataset(
                {
                    ...
                    "generator_target": generator_target_dataset,
                    "discriminator_target": discriminator_target_dataset,
                    "nsentences": NumSamplesDataset(),
                    "ntokens": NumelDataset(dataset, reduce=True),
                    "sample_size": NumelDataset(dataset, reduce=True),
                },
                sizes=[dataset.sizes],
            ),
            sort_order=[
